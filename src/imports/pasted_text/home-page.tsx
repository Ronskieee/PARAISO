...
export function Home() {
  ...
  // Re-read localStorage on mount, focus, and storage changes
  useEffect(() => {
    const syncBooking = () => {
  ...
  // Countdown — only runs if there's a booked date
  useEffect(() => {
  ...
  const highlights = [
    { image: 'https://images.unsplash.com/photo-1535961652354-923cb08225a7?w=400&auto=format&fit=crop&q=60', title: 'Paraiso 2026', description: 'Where every celebration feels like paradise' },
    ...
    { image: 'https://images.unsplash.com/photo-1583939003579-730e3918a45a?w=400&auto=format&fit=crop&q=60', title: 'Wedding Celebrations', description: 'Timeless weddings crafted with love and elegance' },
    { image: 'https://images.unsplash.com/photo-1768508950719-4d76978fdf44?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080', title: 'Elegant Social Gathering', description: 'Unforgettable events for every social occasion' },
  ...
  const formattedDate = bookedDate
    ? new Date(bookedDate + 'T00:00:00').toLocaleDateString('en-PH', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
    : '';
      ...
      <div className="relative z-10 container mx-auto px-4 text-center">
        ...
        {hasBooking && (
          <p className="text-yellow-400 text-lg mb-4">
            🎉 Your {bookedEvent} at {bookedVenue}
          </p>
        )}
        <p className="text-white/70 text-sm mb-10">{formattedDate}</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto mb-12">
          {Object.entries(timeLeft).map(([unit, value]) => (
            <div key={unit} className="bg-black/50 backdrop-blur-sm border-2 border-yellow-500/50 rounded-lg p-4 md:p-6">
              <div className="text-3xl md:text-5xl font-bold text-yellow-400">{value.toString().padStart(2, '0')}</div>
              <div className="text-xs md:text-base text-white uppercase mt-2">{unit}</div>
            </div>
          ))}
        </div>
        <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4">
          <Link to="/registration" className="px-8 py-4 bg-gradient-to-r from-yellow-500 to-amber-600 text-black font-bold text-lg rounded-lg hover:from-yellow-400 hover:to-amber-500 transition-all shadow-lg flex items-center space-x-2">
            <span>Book now!</span>
            <ArrowRight className="w-5 h-5" />
          </Link>
          <Link to="/schedule" className="px-8 py-4 bg-white/10 backdrop-blur-sm text-white font-bold text-lg rounded-lg border-2 border-white/30 hover:bg-white/20 transition-all flex items-center space-x-2">
  ...
  const highlightsSection = (
    <section className="relative py-16 overflow-hidden">
      ...
      <div className="relative z-20 container mx-auto px-4 flex flex-col items-center text-center">
        <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 tracking-wide uppercase">Event Highlights</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 w-full max-w-6xl">
          {highlights.map((highlight, index) => (
            <div key={index} className="group relative overflow-hidden rounded-xl shadow-xl cursor-pointer h-60">
              <ImageWithFallback src={highlight.image} alt={highlight.title} className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500" />
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-black' : 'bg-white'}`}>
      ...
      <section className="relative flex flex-col overflow-hidden">
        ...
        <div className="relative z-20 container mx-auto px-4 pt-28 pb-20 flex flex-col items-center text-center">
          ...
          <p className="text-xl md:text-3xl text-white mb-8">Where every celebration feels like paradise</p>
      ...
      {/* ── if has booking → Countdown first, then Highlights ── */}
      {/* ── NO booking → Highlights only, NO countdown ── */}
      {hasBooking ? (
        <>
          {countdownSection}
          {highlightsSection}
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat, index) => {
              ...
              return (
                <div key={index} className={`text-center p-6 md:p-8 rounded-lg border transition-all ${darkMode ? 'bg-black/50 backdrop-blur-sm border-yellow-500/30 hover:border-yellow-500' : 'bg-white border-yellow-300 hover:border-yellow-500 shadow-md'}`}>
      ...
      <section className="py-20 bg-gradient-to-br from-yellow-500 via-amber-600 to-yellow-500 text-black">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4">
            <Link to="/registration" className="px-10 py-4 bg-black text-yellow-400 font-bold text-lg rounded-lg hover:bg-gray-900 transition-all shadow-xl">Secure Your Spot</Link>
            <Link to="/speakers" className="px-10 py-4 bg-white/20 backdrop-blur-sm text-black font-bold text-lg rounded-lg border-2 border-black/20 hover:bg-white/40 transition-all">Meet Our BOD</Link>
          </div>
        </div>
      </section>
